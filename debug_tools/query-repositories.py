...
Usage:
    python query-repositories.py [--owner OWNER] [--repository REPO] [--container NAME]
"""
import argparse
import subprocess
import sys
...
def main():
    parser = argparse.ArgumentParser(description="Query the repositories table")
    ...
    label = f"repositories ({', '.join(conditions)})" if conditions else "repositories"
    sql = f"SELECT owner, repository, url, kalynx_review_head FROM repositories{where} ORDER BY owner, repository;"
